...
Public Class DLL_Dashboard
	...
	Public Function Insert_ProfileCreator(ByVal email As String, ByVal VerificationCode As String) As Byte
		Try
			...
			sqlconn.Open()
			Dim sqlcom As New SqlCommand("exec dbo.sp_dash_InsertPreRegister '" +
	...
	Public Function Update_UpdatePersonalInformation(ByVal name As String, ByVal lastname As String,
													 ByVal type As Byte, ByVal engName As String, ByVal _
													 engLastname As String, ByVal bio As String, ByVal engBio As String,
													 ByVal birthDay As String, ByVal address As String, ByVal web As String,
													 ByVal mobileShow As Boolean, ByVal emailShow As Boolean,
													 ByVal tell As String, ByVal degree As Byte,
													 ByVal filed As String, ByVal tw As String, ByVal Insta As String,
													 ByVal FB As String, ByVal IMDB As String, ByVal Google As String,
													 ByVal countryCode As Integer, ByVal state As String, ByVal city As String,
	...
	Public Function Update_UpdateVerificationCode(ByVal email As String, ByVal VerificationCode As String) As Byte
		Try
			If sqlconn.State = ConnectionState.Open Then sqlconn.Close()
			sqlconn.Open()
			Dim sqlcom As New SqlCommand("exec dbo.sp_dash_UpdateVerificationCode '" +
										 email + "','" + VerificationCode + "'", sqlconn)
			Return sqlcom.ExecuteNonQuery()
			sqlconn.Close()
		Catch ex As Exception
	...
	Public Function CheckLogin(ByVal email As String, ByVal pass As String) As Boolean
		Try
			...
			sqlconn.Open()
			Dim Encrypt As New Encryptor
			Dim sqlcom As New SqlCommand("select count(*) from tbl_ProfileCreator where email='" + email + "' and password='" +
	...
	Public Function Update_PictureProfileHeader(ByVal email As String, ByVal photo As String, ByVal flag As Boolean) As String
		Try
			...
			Dim sqlcom As New SqlCommand("exec dbo.sp_dash_updatePictureProfileHeader " +
	...
	Public Function Update_Mobile(ByVal email As String, ByVal mobile As String) As Byte
		Try
			If sqlconn.State = ConnectionState.Open Then sqlconn.Close()
			sqlconn.Open()
			Dim sqlcom As New SqlCommand("exec dbo.sp_dash_UpdateMobile '" +
	...
	Public Function Update_MeliCard(ByVal email As String, ByVal MeliCardPic As String, ByVal BirthCertificatePic As String) As Byte
		Try
			If sqlconn.State = ConnectionState.Open Then sqlconn.Close()
			sqlconn.Open()
			Dim sqlcom As New SqlCommand("exec dbo.sp_dash_UpdateMeliCard '" +
	...
	Public Function Update_Username(ByVal username As String, ByVal pass As String, ByVal email As String) As String
		Try
			If sqlconn.State = ConnectionState.Open Then sqlconn.Close()
			sqlconn.Open()
			Dim sqlcom As New SqlCommand("declare @pass varchar(50) set @pass = (SELECT TOP 1 [password] FROM tbl_ProfileCreator WHERE email = '" & email.ToString & "'); exec dbo.sp_dash_UpdateUsernamePassword '" + username + "',@pass,'" + email + "'", sqlconn)
			Return sqlcom.ExecuteNonQuery()
			sqlconn.Close()
		Catch ex As Exception
		Finally
	...
	Public Function CheckStructureUsername(ByVal username As String) As Byte
		Try
			If username.Length > 6 Then
				...
				Dim pattern As String = "[^a-zA-Z0-9]" ' Allows only letters and numbers
				Dim regex As New Regex(pattern)
				If regex.IsMatch(username) Then Return 5
				' other
	...
	Public Function SendForgotPassword(Email As String) As Boolean
		...
		Try
			...
			If sqlcom.ExecuteScalar.ToString <> "" Then
				E.SendMail(Email, String.Format("کلمه عبور شما: {0} می باشد لطفا از لینک زیر اقدام به ورود محیط کاربری خود کنید.",
							Encrypt.DecryptText(sqlcom.ExecuteScalar, "IfP_COM")), "ارسال کلمه عبور",
